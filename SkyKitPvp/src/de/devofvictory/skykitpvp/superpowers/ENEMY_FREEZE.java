...
public class ENEMY_FREEZE implements SuperPowerExecutor{
	...
	public void runSuperPower(Player p) {
		...
		if (!timeout.contains(p)) {
			...
			int effectLengh = Integer.parseInt(kitLevel.getVariableValue("effectDuration"));
		...
		for (Entity en : p.getNearbyEntities(radius, radius, radius)) {
			if (en instanceof Player) {
				...
				if (near != p) {
					...
					PotionEffect slowness = new PotionEffect(PotionEffectType.SLOW, effectLengh*20, 255, false, false, false);
					near.addPotionEffect(slowness);
					
					Variables.setLastDamager(near, p);
